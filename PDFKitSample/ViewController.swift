...
//  PDFKitSample
//
//  Created by xeadmin on 23/08/18.
...
class ViewController: UIViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        guard let url = URL(string:"https://carlosicaza.com/swiftbooks/SwiftLanguage.pdf") else { return }
        activityIndicator.startAnimating()
        let pdfDocument = try! PDFDocument(data: Data(contentsOf: url))
        activityIndicator.stopAnimating()
        pdfView.document = pdfDocument
        //        pdfView.autoScales = true
        pdfView.displayMode = .singlePageContinuous
        pdfView.autoScales = true
        pdfView.backgroundColor = UIColor.cyan
